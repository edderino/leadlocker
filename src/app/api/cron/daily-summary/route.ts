...
async function handleDailySummaryCron(request: NextRequest) {
  try {
    ...
    if (!cronSecret || cronSecret !== expectedSecret) {
      console.error(
        "[Cron] Unauthorized access attempt - invalid or missing x-cron-secret"
      );
      return NextResponse.json(
    ...
    // Resolve client for this user (Phase 1 single-client)
    ...
    if (clientError || !client) {
      console.error("[Cron] Client not found for user", userId, clientError);
      throw new Error("Client not found for daily summary");
    }
    ...
    if (error) {
      console.error(
        "[Cron] Failed to fetch leads from database:",
        error.message
      );
    ...
    if (!recipient) {
      console.error("[Cron] No recipient phone configured for client");
    ...
    log("[Cron] SMS sent successfully", { length: smsBody.length, recipient });
